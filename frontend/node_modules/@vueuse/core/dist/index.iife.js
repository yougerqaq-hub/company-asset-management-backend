(function(exports, _vueuse_shared, vue) {
	...
	function computedAsync(evaluationCallback, initialState, optionsOrRef) {
		...
		const { lazy = false, flush = "sync", evaluating = void 0, shallow = true, onError = (_globalThis$reportErr = globalThis.reportError) !== null && _globalThis$reportErr !== void 0 ? _globalThis$reportErr : _vueuse_shared.noop } = options;
	...
	*/
	function createReusableTemplate(options = {}) {
		...
		const define = (0, vue.defineComponent)({
			name: `${name}.define`,
			setup(_, { slots }) {
				return () => {
		...
		return (0, _vueuse_shared.makeDestructurable)({
			define,
			reuse
		}, [define, reuse]);
	}
	function keysToCamelKebabCase(obj) {
		const newObj = {};
		for (const key in obj) newObj[(0, _vueuse_shared.camelize)(key)] = obj[key];
		return newObj;
	}
	//#endregion
	...
	*/
	...
	const defaultWindow = _vueuse_shared.isClient ? window : void 0;
	const defaultDocument = _vueuse_shared.isClient ? window.document : void 0;
	const defaultNavigator = _vueuse_shared.isClient ? window.navigator : void 0;
	const defaultLocation = _vueuse_shared.isClient ? window.location : void 0;
	//#endregion
	...
	*/
	...
	function useEventListener(...args) {
		...
		const firstParamTargets = (0, vue.computed)(() => {
			const test = (0, _vueuse_shared.toArray)((0, vue.toValue)(args[0])).filter((e) => e != null);
			return test.every((e) => typeof e !== "string") ? test : void 0;
		});
		return (0, _vueuse_shared.watchImmediate)(() => {
			var _firstParamTargets$va, _firstParamTargets$va2;
			return [
				(_firstParamTargets$va = (_firstParamTargets$va2 = firstParamTargets.value) === null || _firstParamTargets$va2 === void 0 ? void 0 : _firstParamTargets$va2.map((e) => unrefElement(e))) !== null && _firstParamTargets$va !== void 0 ? _firstParamTargets$va : [defaultWindow].filter((e) => e != null),
				(0, _vueuse_shared.toArray)((0, vue.toValue)(firstParamTargets.value ? args[1] : args[0])),
				(0, _vueuse_shared.toArray)((0, vue.unref)(firstParamTargets.value ? args[2] : args[1])),
				(0, vue.toValue)(firstParamTargets.value ? args[3] : args[2])
			];
		}, ([raw_targets, raw_events, raw_listeners, raw_options], _, onCleanup) => {
			if (!(raw_targets === null || raw_targets === void 0 ? void 0 : raw_targets.length) || !(raw_events === null || raw_events === void 0 ? void 0 : raw_events.length) || !(raw_listeners === null || raw_listeners === void 0 ? void 0 : raw_listeners.length)) return;
			const optionsClone = (0, _vueuse_shared.isObject)(raw_options) ? { ...raw_options } : raw_options;
	...
	function onClickOutside(target, handler, options = {}) {
		...
		if (!window) return controls ? {
			stop: _vueuse_shared.noop,
			cancel: _vueuse_shared.noop,
			trigger: _vueuse_shared.noop
		} : _vueuse_shared.noop;
		if (_vueuse_shared.isIOS && !_iOSWorkaround) {
			_iOSWorkaround = true;
			const listenerOptions = { passive: true };
			Array.from(window.document.body.children).forEach((el) => el.addEventListener("click", _vueuse_shared.noop, listenerOptions));
			window.document.documentElement.addEventListener("click", _vueuse_shared.noop, listenerOptions);
		}
	...
	*/
	function useMutationObserver(target, callback, options = {}) {
		...
		const targets = (0, vue.computed)(() => {
			const value = (0, vue.toValue)(target);
			const items = (0, _vueuse_shared.toArray)(value).map(unrefElement).filter(_vueuse_shared.notNullish);
			return new Set(items);
		});
		const stopWatch = (0, vue.watch)(targets, (newTargets) => {
			cleanup();
	...
	*/
	function onElementRemoval(target, callback, options = {}) {
		...
		if (!window || !document) return _vueuse_shared.noop;
		let stopFn;
		...
		(0, _vueuse_shared.tryOnScopeDispose)(stopHandle);
		return stopHandle;
	}
	//#endregion
	...
	*/
	function useRafFn(fn, options = {}) {
		...
		if (immediate) resume();
		(0, _vueuse_shared.tryOnScopeDispose)(pause);
		return {
	...
	*/
	function useAnimate(target, keyframes, options) {
		let config;
		let animateOptions;
		if ((0, _vueuse_shared.isObject)(options)) {
			config = options;
			animateOptions = (0, _vueuse_shared.objectOmit)(options, [
				"window",
				"immediate",
				"commitStyles",
				"persist",
		...
		(0, _vueuse_shared.tryOnMounted)(() => update(true), false);
		(0, _vueuse_shared.tryOnScopeDispose)(cancel);
	...
	*/
	function useAsyncQueue(tasks, options) {
		const { interrupt = true, onError = _vueuse_shared.noop, onFinished = _vueuse_shared.noop, signal } = options || {};
		const promiseState = {
			aborted: "aborted",
			fulfilled: "fulfilled",
			pending: "pending",
		...
		const result = (0, vue.reactive)(initialResult);
	...
	*/
	function useAsyncState(promise, initialState, options) {
		var _globalThis$reportErr;
		const { immediate = true, delay = 0, onError = (_globalThis$reportErr = globalThis.reportError) !== null && _globalThis$reportErr !== void 0 ? _globalThis$reportErr : _vueuse_shared.noop, onSuccess = _vueuse_shared.noop, resetOnExecute = true, shallow = true, throwError } = options !== null && options !== void 0 ? options : {};
		...
		async function execute(delay = 0, ...args) {
			...
			if (delay > 0) await (0, _vueuse_shared.promiseTimeout)(delay);
		...
		function waitUntilIsLoaded() {
			return new Promise((resolve, reject) => {
				(0, _vueuse_shared.until)(isLoading).toBe(false).then(() => resolve(shell)).catch(reject);
			});
		}
		return {
			...shell,
	...
	function useBase64(target, options) {
		...
		function execute() {
			if (!_vueuse_shared.isClient) return;
	...
	*/
	...
	function useBluetooth(options) {
		...
		return {
			isSupported,
			isConnected: (0, vue.shallowReadonly)(isConnected),
			device,
			requestDevice,
			server,
			error
	...
	function useSSRWidth() {
		const ssrWidth = (0, vue.hasInjectionContext)() ? (0, _vueuse_shared.injectLocal)(ssrWidthSymbol, null) : null;
		return typeof ssrWidth === "number" ? ssrWidth : void 0;
	}
	function provideSSRWidth(width, app) {
		if (app !== void 0) app.provide(ssrWidthSymbol, width);
		else (0, _vueuse_shared.provideLocal)(ssrWidthSymbol, width);
	}
	//#endregion
	...
	function useMediaQuery(query, options = {}) {
		...
		(0, vue.watchEffect)(() => {
			if (ssrSupport.value) {
				...
				const queryStrings = (0, vue.toValue)(query).split(",");
				matches.value = queryStrings.some((queryString) => {
					const not = queryString.includes("not all");
					const minWidth = queryString.match(/\(\s*min-width:\s*(-?\d+(?:\.\d*)?[a-z]+\s*)\)/);
					const maxWidth = queryString.match(/\(\s*max-width:\s*(-?\d+(?:\.\d*)?[a-z]+\s*)\)/);
					let res = Boolean(minWidth || maxWidth);
					if (minWidth && res) res = ssrWidth >= (0, _vueuse_shared.pxValue)(minWidth[1]);
					if (maxWidth && res) res = ssrWidth <= (0, _vueuse_shared.pxValue)(maxWidth[1]);
					return not ? !res : res;
				});
				return;
			}
	...
	*/
	function useBreakpoints(breakpoints, options = {}) {
		function getValue(k, delta) {
			...
			if (delta != null) v = (0, _vueuse_shared.increaseWithUnit)(v, delta);
		...
		if (ssrSupport) (0, _vueuse_shared.tryOnMounted)(() => mounted.value = !!window);
		function match(query, size) {
			if (!mounted.value && ssrSupport) return query === "min" ? ssrWidth >= (0, _vueuse_shared.pxValue)(size) : ssrWidth <= (0, _vueuse_shared.pxValue)(size);
		...
		function current() {
			const points = Object.keys(breakpoints).map((k) => [
				k,
				shortcutMethods[k],
				(0, _vueuse_shared.pxValue)(getValue(k))
			]).sort((a, b) => a[2] - b[2]);
	...
	*/
	function useBroadcastChannel(options) {
		...
		const channel = (0, vue.shallowRef)();
		...
		if (isSupported.value) (0, _vueuse_shared.tryOnMounted)(() => {
			error.value = null;
	...
	*/
	function useBrowserLocation(options = {}) {
		...
		for (const [key, ref] of (0, _vueuse_shared.objectEntries)(refs)) (0, vue.watch)(ref, (value) => {
	...
	function useCached(refValue, comparator = (newSourceValue, cachedValue) => newSourceValue === cachedValue, options) {
		...
		const cachedValue = (0, _vueuse_shared.createRef)(refValue.value, deepRefs);
	...
	function usePermission(permissionDesc, options = {}) {
		...
		const query = (0, _vueuse_shared.createSingletonPromise)(async () => {
			if (!isSupported.value) return;
	...
	function useClipboard(options = {}) {
		...
		const timeout = (0, _vueuse_shared.useTimeoutFn)(() => copied.value = false, copiedDuring, { immediate: false });
		let lastLegacyId = 0;
		...
		async function copy(value) {
			const resolvedValue = value !== null && value !== void 0 ? value : (0, vue.toValue)(source);
		...
		return {
			copyPending: (0, vue.shallowReadonly)(copyPending),
			isSupported,
	...
	function useClipboardItems(options = {}) {
		...
		const content = (0, vue.shallowRef)([]);
		const copied = (0, vue.shallowRef)(false);
		const timeout = (0, _vueuse_shared.useTimeoutFn)(() => copied.value = false, copiedDuring, { immediate: false });
	...
	*/
	function useStorage(key, defaults, storage, options = {}) {
		...
		const { pause: pauseWatch, resume: resumeWatch } = (0, _vueuse_shared.watchPausable)(data, (newValue) => write(newValue), {
			flush,
			deep,
			eventFilter
		});
		...
		*/
		...
		if (initOnMounted) (0, _vueuse_shared.tryOnMounted)(() => {
			firstMounted = true;
			update();
		});
		else update();
	...
	*/
	function useColorMode(options = {}) {
		...
		const store = storageRef || (storageKey == null ? (0, _vueuse_shared.toRef)(initialValue) : useStorage(storageKey, initialValue, storage, {
			window,
		...
		(0, _vueuse_shared.tryOnMounted)(() => onChanged(state.value));
	...
	*/
	function useConfirmDialog(revealed = (0, vue.shallowRef)(false)) {
		const confirmHook = (0, _vueuse_shared.createEventHook)();
		const cancelHook = (0, _vueuse_shared.createEventHook)();
		const revealHook = (0, _vueuse_shared.createEventHook)();
		let _resolve = _vueuse_shared.noop;
		const reveal = (data) => {
	...
	*/
	function useCountdown(initialCountdown, options = {}) {
		...
		const { scheduler = (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 1e3, { immediate: false }), onTick, onComplete } = options;
	...
	function useCssSupports(...args) {
		...
		return { isSupported: (0, vue.computed)(() => {
			if (!_vueuse_shared.isClient || !isMounted.value) return ssrValue;
			return args.length === 2 ? window === null || window === void 0 ? void 0 : window.CSS.supports((0, vue.toValue)(prop), (0, vue.toValue)(value)) : window === null || window === void 0 ? void 0 : window.CSS.supports((0, vue.toValue)(prop));
		}) };
	}
	//#endregion
	...
	*/
	...
	function useCurrentElement(rootComponent) {
		...
		const currentElement = (0, _vueuse_shared.computedWithControl)(() => null, () => rootComponent ? unrefElement(rootComponent) : vm.proxy.$el);
	...
	*/
	function useCycleList(list, options) {
		...
		const listRef = (0, _vueuse_shared.toRef)(list);
	...
	function useManualRefHistory(source, options = {}) {
		...
		function _createHistoryRecord() {
			return (0, vue.markRaw)({
				...
				timestamp: (0, _vueuse_shared.timestamp)()
			});
		}
	...
	*/
	function useRefHistory(source, options = {}) {
		...
		const { eventFilter: composedFilter, pause, resume: resumeTracking, isActive: isTracking } = (0, _vueuse_shared.pausableFilter)(eventFilter);
		let lastRawValue = source.value;
		const { ignoreUpdates, ignorePrevAsyncUpdates, stop } = (0, _vueuse_shared.watchIgnorable)(source, commit, {
			deep,
			flush,
	...
	*/
	function useDebouncedRefHistory(source, options = {}) {
		const filter = options.debounce ? (0, _vueuse_shared.debounceFilter)(options.debounce) : void 0;
	...
	*/
	function useDeviceMotion(options = {}) {
		const { window = defaultWindow, requestPermissions = false, eventFilter = _vueuse_shared.bypassFilter } = options;
		...
		function init() {
			if (window) {
				const onDeviceMotion = (0, _vueuse_shared.createFilterWrapper)(eventFilter, (event) => {
	...
	*/
	function useDevicePixelRatio(options = {}) {
		...
		let stop = _vueuse_shared.noop;
		if (window) stop = (0, _vueuse_shared.watchImmediate)(query, () => pixelRatio.value = window.devicePixelRatio);
		return {
			pixelRatio: (0, vue.shallowReadonly)(pixelRatio),
			stop
		};
	}
	//#endregion
	...
	*/
	function useDevicesList(options = {}) {
		...
		const devices = (0, vue.shallowRef)([]);
	...
	*/
	function useDraggable(target, options = {}) {
		...
		return {
			...(0, _vueuse_shared.toRefs)(position),
			position,
	...
	*/
	function useResizeObserver(target, callback, options = {}) {
		...
		const stopWatch = (0, vue.watch)(targets, (els) => {
			cleanup();
	...
	*/
	function useElementByPoint(options) {
		...
		const isSupported = /* @__PURE__ */ useSupported(() => {
			if ((0, vue.toValue)(multiple)) return document && "elementsFromPoint" in document;
	...
	function useElementSize(target, initialSize = {
		...
		const { stop: stop1 } = useResizeObserver(target, ([entry]) => {
			...
			if (window && isSVG.value) {
				...
				const formatBoxSize = (0, _vueuse_shared.toArray)(boxSize);
	...
	*/
	function useIntersectionObserver(target, callback, options = {}) {
		...
		const targets = (0, vue.computed)(() => {
			const _target = (0, vue.toValue)(target);
			return (0, _vueuse_shared.toArray)(_target).map(unrefElement).filter(_vueuse_shared.notNullish);
		});
		let cleanup = _vueuse_shared.noop;
	...
	function useElementVisibility(element, options = {}) {
		...
		const isVisible = (0, vue.shallowRef)(initialValue);
		const observerController = useIntersectionObserver(element, (intersectionObserverEntries) => {
			...
			if (once) (0, _vueuse_shared.watchOnce)(isVisible, () => {
	...
	function useEventBus(key) {
		function on(listener) {
			...
			(0, _vueuse_shared.tryOnScopeDispose)(_off);
			return _off;
		}
	...
	*/
	function useEventSource(url, events = [], options = {}) {
		...
		const eventSource = (0, vue.shallowRef)(null);
		const error = (0, vue.shallowRef)(null);
		const urlRef = (0, _vueuse_shared.toRef)(url);
		...
		const close = () => {
			if (_vueuse_shared.isClient && eventSource.value) {
		...
		(0, _vueuse_shared.tryOnScopeDispose)(close);
		return {
			eventSource,
			event,
			data,
	...
	*/
	...
	function useFavicon(newIcon = null, options = {}) {
		...
		const favicon = (0, _vueuse_shared.toRef)(newIcon);
	...
	*/
	function isFetchOptions(obj) {
		return obj && (0, _vueuse_shared.containsProp)(obj, "immediate", "refetch", "initialData", "timeout", "beforeFetch", "afterFetch", "onFetchError", "fetch", "updateDataOnError");
	}
	...
	function useFetch(url, ...args) {
		...
		const responseEvent = (0, _vueuse_shared.createEventHook)();
		const errorEvent = (0, _vueuse_shared.createEventHook)();
		const finallyEvent = (0, _vueuse_shared.createEventHook)();
		...
		if (timeout) timer = (0, _vueuse_shared.useTimeoutFn)(abort, timeout, { immediate: false });
		...
		const refetch = (0, _vueuse_shared.toRef)(options.refetch);
		(0, vue.watch)([refetch, (0, _vueuse_shared.toRef)(url)], ([refetch]) => refetch && execute(), { deep: true });
		const shell = {
			isFinished: (0, vue.shallowReadonly)(isFinished),
			isFetching: (0, vue.shallowReadonly)(isFetching),
			statusCode,
			response,
			error,
			data,
		...
		function setMethod(method) {
			return (payload, payloadType) => {
				if (!isFetching.value) {
					...
					if ((0, vue.isRef)(config.payload)) (0, vue.watch)([refetch, (0, _vueuse_shared.toRef)(config.payload)], ([refetch]) => refetch && execute(), { deep: true });
					return {
						...shell,
						then(onFulfilled, onRejected) {
		...
		function waitUntilFinished() {
			return new Promise((resolve, reject) => {
				(0, _vueuse_shared.until)(isFinished).toBe(true).then(() => resolve(shell)).catch(reject);
			});
		}
	...
	*/
	function useFileDialog(options = {}) {
		...
		const { on: onChange, trigger: changeTrigger } = (0, _vueuse_shared.createEventHook)();
		const { on: onCancel, trigger: cancelTrigger } = (0, _vueuse_shared.createEventHook)();
		...
		const applyOptions = (options) => {
			...
			if (!el) return;
			el.multiple = (0, vue.toValue)(options.multiple);
			el.accept = (0, vue.toValue)(options.accept);
			el.webkitdirectory = (0, vue.toValue)(options.directory);
			if ((0, _vueuse_shared.hasOwn)(options, "capture")) el.capture = (0, vue.toValue)(options.capture);
		};
	...
	*/
	function useFullscreen(target, options = {}) {
		...
		(0, _vueuse_shared.tryOnMounted)(handlerCallback, false);
		if (autoExit) (0, _vueuse_shared.tryOnScopeDispose)(exit);
		return {
			isSupported,
			isFullscreen,
			enter,
	...
	*/
	...
	function useGamepad(options = {}) {
		...
		const onConnectedHook = (0, _vueuse_shared.createEventHook)();
		const onDisconnectedHook = (0, _vueuse_shared.createEventHook)();
	...
	*/
	function useIdle(timeout = oneMinute, options = {}) {
		const { initialState = false, listenForVisibilityChange = true, events = defaultEvents$1, window = defaultWindow, eventFilter = (0, _vueuse_shared.throttleFilter)(50) } = options;
		const idle = (0, vue.shallowRef)(initialState);
		const lastActive = (0, vue.shallowRef)((0, _vueuse_shared.timestamp)());
		...
		const onEvent = (0, _vueuse_shared.createFilterWrapper)(eventFilter, () => {
			lastActive.value = (0, _vueuse_shared.timestamp)();
			reset();
		});
		if (window) {
	...
	*/
	function useScroll(element, options = {}) {
		const { throttle = 0, idle = 200, onStop = _vueuse_shared.noop, onScroll = _vueuse_shared.noop, offset = {
			left: 0,
			right: 0,
			top: 0,
			bottom: 0
		...
		const onScrollEndDebounced = (0, _vueuse_shared.useDebounceFn)(onScrollEnd, throttle + idle);
		...
		useEventListener(element, "scroll", throttle ? (0, _vueuse_shared.useThrottleFn)(onScrollHandler, throttle, true, false) : onScrollHandler, eventListenerOptions);
		(0, _vueuse_shared.tryOnMounted)(() => {
			try {
				const _element = (0, vue.toValue)(element);
				if (!_element) return;
	...
	*/
	function useInfiniteScroll(element, onLoadMore, options = {}) {
		...
		(0, _vueuse_shared.tryOnUnmounted)(stop);
		return {
			isLoading,
			reset() {
				(0, vue.nextTick)(() => checkAndLoad());
	...
	function useLiveAnnouncer(options = {}) {
		...
		function announce(message, mode = "polite", timeout) {
			...
			if (element) {
				...
				element.textContent = "";
				(0, vue.nextTick)(() => element.textContent = message);
	...
	*/
	function useMagicKeys(options = {}) {
		const { reactive: useReactive = false, target = defaultWindow, aliasMap = DefaultMagicKeysAliasMap, passive = true, onEventFired = _vueuse_shared.noop } = options;
	...
	*/
	...
	function useMediaControls(target, options = {}) {
		target = (0, _vueuse_shared.toRef)(target);
		options = {
			...defaultOptions,
			...options
		};
		...
		const buffered = (0, vue.shallowRef)([]);
		const tracks = (0, vue.shallowRef)([]);
		...
		const sourceErrorEvent = (0, _vueuse_shared.createEventHook)();
		const playbackErrorEvent = (0, _vueuse_shared.createEventHook)();
		/**
		...
		*/
		(0, vue.watchEffect)(() => {
			...
			else if ((0, _vueuse_shared.isObject)(src)) sources = [src];
		...
		*/
		const { ignoreUpdates: ignoreCurrentTimeUpdates } = (0, _vueuse_shared.watchIgnorable)(currentTime, (time) => {
			const el = (0, vue.toValue)(target);
			if (!el) return;
			el.currentTime = time;
		});
		...
		* a function
		*/
		const { ignoreUpdates: ignorePlayingUpdates } = (0, _vueuse_shared.watchIgnorable)(playing, (isPlaying) => {
			const el = (0, vue.toValue)(target);
			if (!el) return;
		...
		*/
		...
		(0, _vueuse_shared.tryOnScopeDispose)(() => listeners.forEach((listener) => listener()));
		return {
			currentTime,
			duration,
			waiting,
	...
	*/
	function useMouseInElement(target, options = {}) {
		...
		const targetRef = (0, vue.shallowRef)(target !== null && target !== void 0 ? target : window === null || window === void 0 ? void 0 : window.document.body);
	...
	*/
	function useMousePressed(options = {}) {
		...
		const target = (0, vue.computed)(() => unrefElement(options.target) || window);
		const listenerOptions = {
			passive: true,
			capture
		};
	...
	*/
	function useNavigatorLanguage(options = {}) {
		...
		const language = (0, vue.shallowRef)(navigator === null || navigator === void 0 ? void 0 : navigator.language);
	...
	*/
	function useNetwork(options = {}) {
		...
		return {
			isSupported,
			isOnline: (0, vue.shallowReadonly)(isOnline),
			saveData: (0, vue.shallowReadonly)(saveData),
			offlineAt: (0, vue.shallowReadonly)(offlineAt),
			onlineAt: (0, vue.shallowReadonly)(onlineAt),
			downlink: (0, vue.shallowReadonly)(downlink),
			downlinkMax: (0, vue.shallowReadonly)(downlinkMax),
			effectiveType: (0, vue.shallowReadonly)(effectiveType),
			rtt: (0, vue.shallowReadonly)(rtt),
			type: (0, vue.shallowReadonly)(type)
		};
	}
	//#endregion
	...
	function useNow(options = {}) {
		...
		const now = (0, vue.shallowRef)(/* @__PURE__ */ new Date());
	...
	*/
	function useObjectUrl(object) {
		...
		(0, _vueuse_shared.tryOnScopeDispose)(release);
		return (0, vue.shallowReadonly)(url);
	}
	//#endregion
	//#region ../math/useClamp/index.ts
	/* @__NO_SIDE_EFFECTS__ */
	function useClamp(value, min, max) {
		if (typeof value === "function" || (0, vue.isReadonly)(value)) return (0, vue.computed)(() => (0, _vueuse_shared.clamp)((0, vue.toValue)(value), (0, vue.toValue)(min), (0, vue.toValue)(max)));
		const _value = (0, vue.ref)(value);
		return (0, vue.computed)({
			get() {
				return _value.value = (0, _vueuse_shared.clamp)(_value.value, (0, vue.toValue)(min), (0, vue.toValue)(max));
			},
			set(value) {
				_value.value = (0, _vueuse_shared.clamp)(value, (0, vue.toValue)(min), (0, vue.toValue)(max));
			}
		});
	}
	//#endregion
	...
	function useOffsetPagination(options) {
		const { total = Number.POSITIVE_INFINITY, pageSize = 10, page = 1, onPageChange = _vueuse_shared.noop, onPageSizeChange = _vueuse_shared.noop, onPageCountChange = _vueuse_shared.noop } = options;
		...
		if ((0, vue.isRef)(page)) (0, _vueuse_shared.syncRef)(page, currentPage, { direction: (0, vue.isReadonly)(page) ? "ltr" : "both" });
		if ((0, vue.isRef)(pageSize)) (0, _vueuse_shared.syncRef)(pageSize, currentPageSize, { direction: (0, vue.isReadonly)(pageSize) ? "ltr" : "both" });
		function prev() {
			currentPage.value--;
		}
		function next() {
	...
	*/
	function useScreenOrientation(options = {}) {
		...
		const orientation = (0, vue.shallowRef)(screenOrientation.type);
	...
	*/
	function useParallax(target, options = {}) {
		...
		const orientation = (0, vue.reactive)(useDeviceOrientation({ window }));
		const screenOrientation = (0, vue.reactive)(useScreenOrientation({ window }));
	...
	function useParentElement(element = useCurrentElement()) {
		...
		(0, _vueuse_shared.tryOnMounted)(update);
	...
	*/
	function usePointer(options = {}) {
		...
		const handler = (event) => {
			...
			state.value = (0, _vueuse_shared.objectPick)(event, keys, false);
		};
		if (target) {
			const listenerOptions = { passive: true };
	...
	*/
	function usePointerLock(target, options = {}) {
		...
		async function lock(e) {
			...
			return await (0, _vueuse_shared.until)(element).toBe(targetElement);
		}
		async function unlock() {
			if (!element.value) return false;
			document.exitPointerLock();
			await (0, _vueuse_shared.until)(element).toBeNull();
			return true;
		}
		return {
			isSupported,
	...
	*/
	function usePointerSwipe(target, options = {}) {
		const targetRef = (0, _vueuse_shared.toRef)(target);
		...
		return {
			isSwiping: (0, vue.shallowReadonly)(isSwiping),
			direction: (0, vue.shallowReadonly)(direction),
	...
	*/
	function usePreferredLanguages(options = {}) {
		const { window = defaultWindow } = options;
		if (!window) return (0, vue.shallowRef)(["en"]);
		const navigator = window.navigator;
		const value = (0, vue.shallowRef)(navigator.languages);
		useEventListener(window, "languagechange", () => {
	...
	*/
	...
	function usePrevious(value, initialValue) {
		...
		(0, vue.watch)((0, _vueuse_shared.toRef)(value), (_, oldValue) => {
	...
	*/
	function useScreenSafeArea() {
		...
		if (_vueuse_shared.isClient) {
			...
			(0, _vueuse_shared.tryOnMounted)(update);
			useEventListener("resize", (0, _vueuse_shared.useDebounceFn)(update), { passive: true });
		}
		function update() {
	...
	* @param options
	*/
	function useScriptTag(src, onLoaded = _vueuse_shared.noop, options = {}) {
		...
		*/
		const loadScript = (waitForScriptLoad) => new Promise((resolve, reject) => {
			...
			let el = document.querySelector(`script[src="${(0, vue.toValue)(src)}"]`);
			if (!el) {
		...
		*/
		const unload = () => {
			...
			const el = document.querySelector(`script[src="${(0, vue.toValue)(src)}"]`);
			if (el) document.head.removeChild(el);
		};
		if (immediate && !manual) (0, _vueuse_shared.tryOnMounted)(load);
		if (!manual) (0, _vueuse_shared.tryOnUnmounted)(unload);
		return {
			scriptTag,
			load,
			unload
	...
	*/
	function useScrollLock(element, initialState = false) {
		...
		let initialOverflow = "";
		(0, vue.watch)((0, _vueuse_shared.toRef)(element), (el) => {
		...
		const lock = () => {
			...
			if (_vueuse_shared.isIOS) stopTouchMoveListener = useEventListener(el, "touchmove", (e) => {
				preventDefault(e);
			}, { passive: false });
		...
		const unlock = () => {
			const el = resolveElement((0, vue.toValue)(element));
			if (!el || !isLocked.value) return;
			if (_vueuse_shared.isIOS) stopTouchMoveListener === null || stopTouchMoveListener === void 0 || stopTouchMoveListener();
			el.style.overflow = initialOverflow;
			elInitialOverflow.delete(el);
			isLocked.value = false;
		};
		(0, _vueuse_shared.tryOnScopeDispose)(unlock);
		return (0, vue.computed)({
			get() {
	...
	*/
	function useSpeechRecognition(options = {}) {
		...
		return {
			isSupported,
			isListening,
			isFinal: (0, vue.shallowReadonly)(isFinal),
			recognition,
			result: (0, vue.shallowReadonly)(result),
			confidence: (0, vue.shallowReadonly)(confidence),
			error,
			toggle,
			start,
			stop
	...
	*/
	function useSpeechSynthesis(text, options = {}) {
		...
		const spokenText = (0, _vueuse_shared.toRef)(text || "");
		const lang = (0, _vueuse_shared.toRef)(options.lang || "en-US");
		const error = (0, vue.shallowRef)(void 0);
		...
		const bindEventsForUtterance = (utterance) => {
			utterance.lang = (0, vue.toValue)(lang);
			utterance.voice = (0, vue.toValue)(options.voice) || null;
			utterance.pitch = (0, vue.toValue)(pitch);
			utterance.rate = (0, vue.toValue)(rate);
			utterance.volume = (0, vue.toValue)(volume);
			utterance.onstart = () => {
	...
	*/
	function useStorageAsync(key, initialValue, storage, options = {}) {
		...
		if (storage) (0, _vueuse_shared.watchWithFilter)(data, async () => {
			try {
	...
	*/
	function useStyleTag(css, options = {}) {
		...
		if (immediate && !manual) (0, _vueuse_shared.tryOnMounted)(load);
		if (!manual) (0, _vueuse_shared.tryOnScopeDispose)(unload);
		return {
			id,
			css: cssRef,
			unload,
			load,
			isLoaded: (0, vue.shallowReadonly)(isLoaded)
		};
	}
	//#endregion
	...
	*/
	function useTemporalNow(options = {}) {
		...
		const timezone = (0, vue.shallowRef)(initialTimezone);
		const calendar = (0, vue.shallowRef)(initialCalendar);
		const now = (0, vue.shallowRef)(TemporalImpl.Now.zonedDateTimeISO(timezone.value).withCalendar(calendar.value));
		function updateNow() {
		...
		(0, vue.watch)([timezone, calendar], updateNow);
	...
	*/
	function useTextDirection(options = {}) {
		...
		const dir = (0, vue.shallowRef)(getValue());
		(0, _vueuse_shared.tryOnMounted)(() => dir.value = getValue());
	...
	*/
	function useTextSelection(options = {}) {
		...
		const { window = defaultWindow } = options;
		const selection = (0, vue.shallowRef)((_window$getSelection = window === null || window === void 0 ? void 0 : window.getSelection()) !== null && _window$getSelection !== void 0 ? _window$getSelection : null);
	...
	*/
	...
	function useTextareaAutosize(options = {}) {
		...
		const { window = defaultWindow } = options;
		const textarea = (0, _vueuse_shared.toRef)(options === null || options === void 0 ? void 0 : options.element);
		const input = (0, _vueuse_shared.toRef)((_options$input = options === null || options === void 0 ? void 0 : options.input) !== null && _options$input !== void 0 ? _options$input : "");
	...
	*/
	function useThrottledRefHistory(source, options = {}) {
		...
		const filter = (0, _vueuse_shared.throttleFilter)(throttle, trailing);
		return { ...useRefHistory(source, {
			...options,
			eventFilter: filter
		}) };
	...
	function useTimeAgo(time, options = {}) {
		const { controls: exposeControls = false, scheduler = (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 3e4) } = options;
	...
	*/
	...
	function useTimeoutPoll(fn, interval, options = {}) {
		...
		const { start } = (0, _vueuse_shared.useTimeoutFn)(loop, interval, { immediate });
		const isActive = (0, vue.shallowRef)(false);
		async function loop() {
			if (!isActive.value) return;
			await fn();
		...
		if (immediate && _vueuse_shared.isClient) resume();
		(0, _vueuse_shared.tryOnScopeDispose)(pause);
		return {
			isActive,
			pause,
	...
	function useTimestamp(options = {}) {
		...
		const ts = (0, vue.shallowRef)((0, _vueuse_shared.timestamp)() + offset);
		const update = () => ts.value = (0, _vueuse_shared.timestamp)() + offset;
	...
	function useTitle(newTitle = null, options = {}) {
		...
		const title = (0, _vueuse_shared.toRef)((_ref = newTitle !== null && newTitle !== void 0 ? newTitle : document === null || document === void 0 ? void 0 : document.title) !== null && _ref !== void 0 ? _ref : null);
	...
	*/
	const TransitionPresets = /* #__PURE__ */ Object.assign({}, { linear: _vueuse_shared.identity }, {
		easeInSine: [
			.12,
			0,
			.39,
	...
	*/
	...
	function normalizeEasing(easing) {
		var _toValue;
		return typeof easing === "function" ? easing : (_toValue = (0, vue.toValue)(easing)) !== null && _toValue !== void 0 ? _toValue : _vueuse_shared.identity;
	}
	/**
	...
	*/
	function useTransition(source, options = {}) {
		...
		(0, vue.watch)(sourceVal, async (to) => {
			...
			const id = ++currentId;
			if (options.delay) await (0, _vueuse_shared.promiseTimeout)((0, vue.toValue)(options.delay));
	...
	*/
	function useUrlSearchParams(mode = "history", options = {}) {
		...
		if (!window) return (0, vue.reactive)(initialValue);
		...
		const { pause, resume } = (0, _vueuse_shared.watchPausable)(state, () => {
	...
	*/
	...
	function useVModel(props, key, emit, options = {}) {
		...
		const getValue = () => (0, _vueuse_shared.isDef)(props[key]) ? cloneFn(props[key]) : defaultValue;
		...
		if (passive) {
			...
			const proxy = (0, vue.ref)(initialValue);
	...
	*/
	function useVibrate(options) {
		...
		const patternRef = (0, _vueuse_shared.toRef)(pattern);
	...
	*/
	function useWakeLock(options = {}) {
		...
		if (isSupported.value) {
			...
			(0, _vueuse_shared.whenever)(() => documentVisibility.value === "visible" && (document === null || document === void 0 ? void 0 : document.visibilityState) === "visible" && requestedType.value, (type) => {
	...
	*/
	function useWebMCP(options) {
		...
		const isRegistered = (0, vue.shallowRef)(false);
		const error = (0, vue.shallowRef)(null);
		let controller;
		function cleanup() {
		...
		function register() {
			cleanup();
			error.value = null;
			if (!isSupported.value || !(0, vue.toValue)(enabled)) return;
			controller = new AbortController();
			try {
				doc.modelContext.registerTool({
					name: (0, vue.toValue)(options.name),
					description: (0, vue.toValue)(options.description),
					inputSchema: (0, vue.toValue)(options.inputSchema),
					annotations: (0, vue.toValue)(options.annotations),
					async execute(args) {
						try {
		...
		(0, vue.watch)([
			isSupported,
			() => (0, vue.toValue)(options.name),
			() => (0, vue.toValue)(options.description),
			() => (0, vue.toValue)(options.inputSchema) ? safeStringify((0, vue.toValue)(options.inputSchema)) : "",
			() => (0, vue.toValue)(options.annotations) ? safeStringify((0, vue.toValue)(options.annotations)) : "",
			() => (0, vue.toValue)(enabled)
		], register, {
			immediate: true,
			flush: "post"
		});
	...
	*/
	function useWebNotification(options = {}) {
		...
		const notification = (0, vue.shallowRef)(null);
		const ensurePermissions = async () => {
			if (!isSupported.value) return;
		...
		const { on: onClick, trigger: clickTrigger } = (0, _vueuse_shared.createEventHook)();
		const { on: onShow, trigger: showTrigger } = (0, _vueuse_shared.createEventHook)();
		const { on: onError, trigger: errorTrigger } = (0, _vueuse_shared.createEventHook)();
		const { on: onClose, trigger: closeTrigger } = (0, _vueuse_shared.createEventHook)();
		...
		if (_requestForPermissions) (0, _vueuse_shared.tryOnMounted)(ensurePermissions);
		(0, _vueuse_shared.tryOnScopeDispose)(close);
		if (isSupported.value && window) {
			const document = window.document;
	...
	*/
	function useWebSocket(url, options = {}) {
		...
		const close = (code = 1e3, reason) => {
			resetRetry();
			if (!_vueuse_shared.isClient && !_vueuse_shared.isWorker || !wsRef.value) return;
			explicitlyClosed = true;
			resetHeartbeat();
		...
		if (options.heartbeat) {
			const { message = DEFAULT_PING_MESSAGE, scheduler = (cb) => (0, _vueuse_shared.useIntervalFn)(cb, 1e3, { immediate: false }), pongTimeout = 1e3 } = resolveNestedOptions(options.heartbeat);
		...
		if (autoClose) {
			if (_vueuse_shared.isClient) useEventListener("beforeunload", () => close(), { passive: true });
			(0, _vueuse_shared.tryOnScopeDispose)(close);
		}
		const open = () => {
			if (!_vueuse_shared.isClient && !_vueuse_shared.isWorker) return;
			close();
			explicitlyClosed = false;
			retried = 0;
			_init();
	...
	*/
	function useWebWorkerFn(fn, options = {}) {
		...
		workerTerminate();
		(0, _vueuse_shared.tryOnScopeDispose)(workerTerminate);
	...
	*/
	...
	exports.isFocusedElementEditable = isFocusedElementEditable;
	...
	exports.useLiveAnnouncer = useLiveAnnouncer;
	...
	Object.keys(_vueuse_shared).forEach(function(k) {
		if (k !== "default" && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
			enumerable: true,
			get: function() {
				return _vueuse_shared[k];
			}
		});
	});
