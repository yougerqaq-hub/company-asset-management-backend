...
import { AUTO_ALIGNMENT, CENTERED_ALIGNMENT, START_ALIGNMENT } from "../defaults.mjs";
...
const FixedSizeGrid = createGrid({
	...
	getColumnOffset: ({ totalColumn, columnWidth, width }, columnIndex, alignment, scrollLeft, _, scrollBarWidth) => {
		...
		if (alignment === "smart") {
			if (scrollLeft >= minOffset - width && scrollLeft <= maxOffset + width) alignment = AUTO_ALIGNMENT;
	...
	getRowOffset: ({ rowHeight, height, totalRow }, rowIndex, align, scrollTop, _, scrollBarWidth) => {
		...
		if (align === "smart") {
			if (scrollTop >= minOffset - height && scrollTop <= maxOffset + height) align = AUTO_ALIGNMENT;
	...
	*/
	...
	validateProps: ({ columnWidth, rowHeight }) => {
		if (!isNumber(columnWidth)) throwError(SCOPE, `
          "columnWidth" must be passed as number,
            instead ${typeof columnWidth} was given.
        `);
		if (!isNumber(rowHeight)) throwError(SCOPE, `
          "columnWidth" must be passed as number,
