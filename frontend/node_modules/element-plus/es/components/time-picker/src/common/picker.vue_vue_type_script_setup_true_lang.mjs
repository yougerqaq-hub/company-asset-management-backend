		...
		const handleKeydownInput = async (event) => {
			...
			if (code === EVENT_CODE.enter || code === EVENT_CODE.numpadEnter) {
				if (!pickerVisible.value) pickerVisible.value = true;
					...
					handleChange();
					pickerVisible.value = false;
				} else if (!isValidValue(parseUserInputToDayjs(displayValue.value))) debugWarn("Picker", `Invalid user input: ${displayValue.value}. Please check the format of the input.`);
				event.preventDefault();
				event.stopPropagation();
				return;
			}
