...
var basic_time_spinner_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		const emitSelectRange = (type) => {
			...
			switch (type) {
				...
				case "minutes":
					...
					break;
				case "seconds": if (secondIndex !== -1) range = [secondIndex, secondIndex + 2];
			}
		...
		const modifyDateField = (type, value) => {
			...
			switch (type) {
				...
				case "minutes":
					...
					break;
				case "seconds": changeTo = props.spinnerDate.hour(hours).minute(minutes).second(value);
			}
		...
		const handleScroll = (type) => {
			...
			const value = Math.min(Math.round((getScrollbarElement(scrollbar.$el).scrollTop - (scrollBarHeight(type) * .5 - 10) / typeItemHeight(type) + 3) / typeItemHeight(type)), type === "hours" ? 23 : 59);
			modifyDateField(type, value);
		};
