...
import { correctlyParseUserInput, getValidDateOfMonth, getValidDateOfQuarter, getValidDateOfYear, isQuarterFullyDisabled, isSelectableQuarterDate, normalizeQuarterDate } from "../utils.mjs";
	...
	setup(__props, { emit: __emit }) {
		...
		const handleMonthPick = async (month, keepOpen) => {
			if (selectionMode.value === "month") {
				innerDate.value = getValidDateOfMonth(innerDate.value, innerDate.value.year(), month, lang.value, disabledDate.value);
		...
		const handleQuarterPick = async (quarter, keepOpen) => {
			if (selectionMode.value === "quarter") {
				innerDate.value = getValidDateOfQuarter(innerDate.value, innerDate.value.year(), quarter, lang.value, disabledDate.value);
				emit(innerDate.value, false);
		...
		const disabledConfirm = computed(() => {
			...
			if (isArray(props.parsedValue)) {
				...
				if (selectionMode.value === "quarters") return props.parsedValue.some((date) => isQuarterFullyDisabled(date, lang.value, disabledDate.value));
		...
		const isValidValue = (date) => {
			...
			if (selectionMode.value === "quarter") return isSelectableQuarterDate(date, lang.value, disabledDate.value);
			return disabledDate.value ? !disabledDate.value(date.toDate()) : true;
		};
		const parseUserInput = (value) => {
			const parsed = correctlyParseUserInput(value, props.format, lang.value);
			if (!["quarter", "quarters"].includes(selectionMode.value)) return parsed;
			const normalize = (date) => normalizeQuarterDate(date, lang.value, disabledDate.value);
			if (isArray(parsed)) return parsed.map(normalize);
			if (!dayjs.isDayjs(parsed) || !parsed.isValid()) return parsed;
		...
		const handleKeyControl = (code) => {
			...
			while (Math.abs(innerDate.value.diff(newDate, "year", true)) < 1) {
				...
				if (keyboardMode.value === "quarter") {
					...
					if (isQuarterFullyDisabled(candidate, lang.value, disabledDate.value)) break;
					result = getValidDateOfQuarter(candidate, candidate.year(), candidate.quarter() - 1, lang.value, disabledDate.value);
				} else {
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass([
				...
				createElementVNode("div", { class: normalizeClass(unref(ppNs).e("body")) }, [
					...
					withDirectives(createElementVNode("div", { class: normalizeClass([unref(dpNs).e("header"), (currentView.value === "year" || currentView.value === "month" || currentView.value === "quarter") && unref(dpNs).em("header", "bordered")]) }, [
					...
					createElementVNode("div", {
						...
						currentView.value === "quarter" ? (openBlock(), createBlock(basic_quarter_table_default, {
							key: 3,
							ref_key: "currentViewRef",
							ref: currentViewRef,
							"selection-mode": selectionMode.value,
