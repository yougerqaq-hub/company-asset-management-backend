import { isArray, isBoolean, isFunction, isString, isUndefined as isUndefined$1 } from "../../../../utils/types.mjs";
import { hasOwn } from "../../../../utils/objects.mjs";
import { NODE_KEY, markNodeData } from "./util.mjs";
...
var Node = class Node {
	...
	insertChild(child, index, batch) {
		...
		if (!(child instanceof Node)) {
			if (!batch) {
				...
				if (!children?.includes(child.data)) {
					if (isUndefined$1(index) || index < 0) children?.push(child.data);
		...
		if (isUndefined$1(index) || index < 0) this.childNodes.push(child);
	...
	getChildren(forceInit = false) {
		...
		if (isUndefined$1(data[children])) data[children] = null;
	...
	updateChildren() {
		...
		newData.forEach((item, index) => {
			...
			if (!!key && oldData.some((data) => data?.["$treeNodeId"] === key)) newDataMap[key] = {
				index,
				data: item
			};
			else newNodes.push({
