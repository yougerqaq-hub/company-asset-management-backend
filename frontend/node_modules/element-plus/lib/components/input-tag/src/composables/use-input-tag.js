...
function useInputTag({ props, emit, formItem }) {
	...
	const handleKeydown = (event) => {
		...
		switch (require_event$1.getEventCode(event)) {
			...
			case require_aria.EVENT_CODE.numpadEnter:
				...
				break;
			case require_aria.EVENT_CODE.backspace: if (!inputValue.value && props.modelValue?.length) {
				event.preventDefault();
				event.stopPropagation();
	...
	const handleKeyup = (event) => {
		...
		switch (require_event$1.getEventCode(event)) {
			case require_aria.EVENT_CODE.space: if (props.trigger === require_aria.EVENT_CODE.space) {
				event.preventDefault();
				event.stopPropagation();
				handleAddTag();
			}
	...
	(0, vue.watch)(() => props.modelValue, () => {
		if (props.validateEvent) formItem?.validate?.(require_event.CHANGE_EVENT).catch(_vue_shared.NOOP);
	});
	return {
		inputRef,
		wrapperRef,
