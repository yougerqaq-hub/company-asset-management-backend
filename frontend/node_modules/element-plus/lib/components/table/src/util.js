...
function parseHeight(height) {
	...
	if ((0, _vue_shared.isString)(height)) {
		if (/^\d+(?:px)?$/.test(height)) return Number.parseInt(height, 10);
		else return height;
	}
	return null;
}
...
function toggleRowStatus(statusArr, row, newVal, tableTreeProps, selectable, rowIndex, rowKey) {
	...
	if (!selectable || isRowSelectable) {
		if (require_types.isBoolean(newVal)) {
...
exports.createTableColumnId = createTableColumnId;
