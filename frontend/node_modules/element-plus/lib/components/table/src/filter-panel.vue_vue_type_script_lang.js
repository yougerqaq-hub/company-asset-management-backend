...
var filter_panel_vue_vue_type_script_lang_default = (0, vue.defineComponent)({
	...
	setup(props) {
		...
		const filterValue = (0, vue.computed)({
			...
			set: (value) => {
				if (props.column) {
					...
					if (!require_types.isPropAbsent(value)) arr.splice(0, 1, value);
					else arr.splice(0, 1);
		...
		const handleSelect = (_filterValue, index) => {
			...
			if (!require_types.isPropAbsent(_filterValue)) confirmFilter([_filterValue]);
			else confirmFilter([]);
			hidden();
		};
