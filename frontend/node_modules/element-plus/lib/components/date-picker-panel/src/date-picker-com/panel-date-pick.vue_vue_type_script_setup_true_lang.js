...
const require_basic_quarter_table = require("./basic-quarter-table.js");
...
var panel_date_pick_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const disabledDate = (0, vue.toRef)(pickerBase.props, "disabledDate");
		...
		const emit = (value, ...args) => {
			...
			else if ((0, _vue_shared.isArray)(value)) {
		...
		const isMultipleType = (0, vue.computed)(() => {
			return selectionMode.value === "dates" || selectionMode.value === "months" || selectionMode.value === "years" || selectionMode.value === "quarters";
		});
		...
		const handleMonthPick = async (month, keepOpen) => {
			if (selectionMode.value === "month") {
				innerDate.value = require_utils$1.getValidDateOfMonth(innerDate.value, innerDate.value.year(), month, lang.value, disabledDate.value);
		...
		const handleQuarterPick = async (quarter, keepOpen) => {
			if (selectionMode.value === "quarter") {
				innerDate.value = require_utils$1.getValidDateOfQuarter(innerDate.value, innerDate.value.year(), quarter, lang.value, disabledDate.value);
				emit(innerDate.value, false);
			} else if (selectionMode.value === "quarters") emit(quarter, keepOpen ?? true);
		};
		const handleYearPick = async (year, keepOpen) => {
			if (selectionMode.value === "year") {
				const data = innerDate.value.startOf("year").year(year);
				innerDate.value = require_utils$1.getValidDateOfYear(data, lang.value, disabledDate.value);
				emit(innerDate.value, false);
			} else if (selectionMode.value === "years") emit(year, keepOpen ?? true);
			else {
				const data = innerDate.value.year(year);
				innerDate.value = require_utils$1.getValidDateOfYear(data, lang.value, disabledDate.value);
				currentView.value = ["quarter", "quarters"].includes(selectionMode.value) ? "quarter" : "month";
				if ([
					"month",
					"year",
					"date",
		...
		const footerVisible = (0, vue.computed)(() => {
			...
			return showDateFooter && isDateView || showYearFooter && isYearView || showMonthFooter && isMonthView || showQuarterFooter && isQuarterView;
		});
		...
		const disabledConfirm = (0, vue.computed)(() => {
			...
			if ((0, _vue_shared.isArray)(props.parsedValue)) {
				if (!props.parsedValue.length) return true;
				if (selectionMode.value === "quarters") return props.parsedValue.some((date) => require_utils$1.isQuarterFullyDisabled(date, lang.value, disabledDate.value));
		...
		const disabledNow = (0, vue.computed)(() => {
			if (!disabledDate.value) return false;
			return disabledDate.value((0, dayjs.default)().locale(lang.value).toDate());
		});
		const changeToNow = () => {
			const nowDate = (0, dayjs.default)().locale(lang.value).toDate();
			isChangeToNow.value = true;
			if ((!disabledDate.value || !disabledDate.value(nowDate)) && checkDateWithinRange(nowDate)) {
		...
		const handleVisibleDateChange = (value) => {
			...
			if (newDate.isValid()) {
				if (disabledDate.value && disabledDate.value(newDate.toDate())) return;
		...
		const isValidValue = (date) => {
			if (!dayjs.default.isDayjs(date) || !date.isValid()) return false;
			if (selectionMode.value === "quarter") return require_utils$1.isSelectableQuarterDate(date, lang.value, disabledDate.value);
			return disabledDate.value ? !disabledDate.value(date.toDate()) : true;
		};
		const parseUserInput = (value) => {
			const parsed = require_utils$1.correctlyParseUserInput(value, props.format, lang.value);
			if (!["quarter", "quarters"].includes(selectionMode.value)) return parsed;
			const normalize = (date) => require_utils$1.normalizeQuarterDate(date, lang.value, disabledDate.value);
			if ((0, _vue_shared.isArray)(parsed)) return parsed.map(normalize);
			if (!dayjs.default.isDayjs(parsed) || !parsed.isValid()) return parsed;
		...
		const handleKeyControl = (code) => {
			...
			while (Math.abs(innerDate.value.diff(newDate, "year", true)) < 1) {
				...
				if (keyboardMode.value === "quarter") {
					const candidate = (0, dayjs.default)(newDate).locale(lang.value);
					if (require_utils$1.isQuarterFullyDisabled(candidate, lang.value, disabledDate.value)) break;
					result = require_utils$1.getValidDateOfQuarter(candidate, candidate.year(), candidate.quarter() - 1, lang.value, disabledDate.value);
				} else {
					if (disabledDate.value && disabledDate.value(newDate)) break;
					result = (0, dayjs.default)(newDate).locale(lang.value);
				}
				innerDate.value = result;
				if (!isMultipleType.value) contextEmit("pick", result, true);
				break;
			}
		};
		...
		return (_ctx, _cache) => {
			return (0, vue.openBlock)(), (0, vue.createElementBlock)("div", { class: (0, vue.normalizeClass)([
				...
				(0, vue.createElementVNode)("div", { class: (0, vue.normalizeClass)((0, vue.unref)(ppNs).e("body")) }, [
					...
					(0, vue.withDirectives)((0, vue.createElementVNode)("div", { class: (0, vue.normalizeClass)([(0, vue.unref)(dpNs).e("header"), (currentView.value === "year" || currentView.value === "month" || currentView.value === "quarter") && (0, vue.unref)(dpNs).em("header", "bordered")]) }, [
					...
					(0, vue.createElementVNode)("div", {
						...
						currentView.value === "quarter" ? ((0, vue.openBlock)(), (0, vue.createBlock)(require_basic_quarter_table.default, {
							key: 3,
							ref_key: "currentViewRef",
							ref: currentViewRef,
							"selection-mode": selectionMode.value,
