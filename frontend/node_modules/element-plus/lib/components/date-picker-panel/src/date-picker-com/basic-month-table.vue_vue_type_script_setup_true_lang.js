	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const handleMonthTableClick = (event) => {
			...
			if (props.selectionMode === "months") {
				...
				const newValue = require_style.hasClass(target, "current") ? require_arrays.castArray(props.parsedValue).filter((d) => d?.year() !== newMonth.year() || d?.month() !== newMonth.month()) : require_arrays.castArray(props.parsedValue).concat([(0, dayjs.default)(newMonth)]);
				emit("pick", newValue);
