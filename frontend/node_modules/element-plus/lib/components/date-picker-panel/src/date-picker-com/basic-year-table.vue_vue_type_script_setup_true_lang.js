...
var basic_year_table_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ (0, vue.defineComponent)({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		const datesInYear = (year, lang) => {
			...
			const numOfDays = firstDay.endOf("year").dayOfYear();
			return require_utils.rangeArr(numOfDays).map((n) => firstDay.add(n, "day").toDate());
		};
		const props = __props;
		const emit = __emit;
		...
		const handleYearTableClick = (event) => {
			...
			if (props.selectionMode === "range") {
				...
				const newValue = require_style.hasClass(target, "current") ? require_arrays.castArray(props.parsedValue).filter((d) => d?.year() !== selectedYear) : require_arrays.castArray(props.parsedValue).concat([vaildYear]);
				emit("pick", newValue);
