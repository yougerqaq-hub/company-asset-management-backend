...
var panel_date_range_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const isValidValue = (date) => {
			return require_utils$1.isValidRange(date) && (disabledDate.value ? !disabledDate.value(date[0].toDate()) && !disabledDate.value(date[1].toDate()) : true);
		};
		const leftPrevYear = () => {
		...
		const handleDateInput = (value, type) => {
			...
			if (parsedValueD.isValid()) {
				if (disabledDate.value && disabledDate.value(parsedValueD.toDate())) return;
				if (type === "min") {
