...
const useCheck = (props, panelState, emit) => {
	...
	const checkboxGroupRef = (0, vue.ref)();
	const { height: virtualListHeight } = (0, _vueuse_core.useElementSize)((0, vue.computed)(() => checkboxGroupRef.value?.$el));
	const virtualListRef = (0, vue.ref)();
	(0, vue.watch)(() => panelState.checked, (val, oldVal) => {
		updateAllChecked();
		if (panelState.checkChangeByUser) {
			const movedKeys = val.concat(oldVal).filter((v) => !val.includes(v) || !oldVal.includes(v));
			emit(require_transfer_panel.CHECKED_CHANGE_EVENT, val, movedKeys);
		} else {
