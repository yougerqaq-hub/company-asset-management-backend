...
var index_vue_vue_type_script_lang_default = (0, vue.defineComponent)({
	...
	setup(props, { emit }) {
		...
		(0, vue.watch)(() => visible.value, (val) => {
			if (val) {
				if (props.boxType !== "prompt") {
					if (state.autofocus) focusStartRef.value = confirmRef.value?.$el ?? rootRef.value;
			...
			if (val) (0, vue.nextTick)().then(() => {
				if (inputRef.value && inputRef.value.$el) {
					if (state.autofocus) focusStartRef.value = getInputElement() ?? rootRef.value;
					else focusStartRef.value = rootRef.value;
				}
			});
			else {
		...
		const overflow = (0, vue.computed)(() => props.overflow);
		const { isDragging } = require_index.useDraggable(rootRef, headerRef, draggable, overflow);
