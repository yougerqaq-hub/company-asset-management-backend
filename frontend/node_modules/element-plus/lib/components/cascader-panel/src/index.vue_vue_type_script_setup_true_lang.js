...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ (0, vue.defineComponent)({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const calculateCheckedValue = () => {
			...
			const nodes = require_utils.sortByOriginalOrder(oldNodes, newNodes);
		...
		const syncCheckedValue = (loaded = false, forced = false) => {
			...
			if (lazy && !loaded) {
				...
				const values = multiple ? require_arrays.castArray(modelValue) : [modelValue];
				const nodes = require_arrays.unique(values.map((val) => store?.getNodeByValue(val, leafOnly)));
		...
		const handleKeyDown = (e) => {
			...
			switch (code) {
				...
				case require_aria$1.EVENT_CODE.numpadEnter: require_utils.checkNode(target);
			}
		};
