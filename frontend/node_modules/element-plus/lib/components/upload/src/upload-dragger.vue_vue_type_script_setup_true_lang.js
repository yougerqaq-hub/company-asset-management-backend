	...
	setup(__props, { emit: __emit }) {
		...
		const getAllFiles = async (entry) => {
			try {
				if (entry.isFile) {
					...
					if (entry.fullPath && !file.webkitRelativePath) Object.defineProperty(file, "webkitRelativePath", { value: entry.fullPath.replace(/^\//, "") });
				...
				if (entry.isDirectory) {
					...
					*/
					...
					return (0, lodash_unified.flatten)(files);
				}
			} catch {
				return [];
			}
		...
		const onDrop = async (e) => {
			...
			if (props.directory) {
				...
				emit("file", (0, lodash_unified.flatten)(allFiles));
				return;
			}
